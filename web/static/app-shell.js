(() => {
    ...
    const severityMap = {
        ...
        notice: { label: 'Норма', tone: 'primary' },
        info: { label: 'Норма', tone: 'primary' },
        low: { label: 'Норма', tone: 'primary' },
        debug: { label: 'Норма', tone: 'primary' },
    };
    ...
    function getSeverityBadgeClass(labelOrSeverity) {
        const label = ['Критично', 'Требует внимания', 'Норма'].includes(labelOrSeverity)
            ? labelOrSeverity
            : getSeverityLabel(labelOrSeverity);
        if (label === 'Критично') return 'text-red-400 bg-red-900/30';
        if (label === 'Требует внимания') return 'text-orange-400 bg-orange-900/30';
        return 'text-emerald-400 bg-emerald-900/30';
    }

    function getStatusBadgeClass(status) {
        const tone = getStatusMeta(status).tone;
        if (tone === 'danger') return 'text-red-400 bg-red-900/20';
        if (tone === 'warning') return 'text-orange-400 bg-orange-900/20';
